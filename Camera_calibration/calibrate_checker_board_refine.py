...
checkerboard_size_row = 27
checkerboard_size_col = 43
...
for fname in tqdm(images):
    ...
    coordinates = fast_peak_local_max(gray_blur_2, min_distance=3, threshold_abs=50)
    if len(coordinates) < (checkerboard_size_row+1) * (checkerboard_size_col+1) * square_size * square_size / 2:
        raise ValueError('Have not found enough coordinates!')
...
with open(f'calibration_refine.json', 'w') as outfile:
