...
with open(f'MeanRGB_Camera_Colorchecker.json', 'r') as f:
    data_Camera_Colorchecker_MeanRGB = json.load(f)
with open(f'measure_specbos.json', 'r') as f:
    data_Color_measure_specbos = json.load(f)

keys_list = list(data_Camera_Colorchecker_MeanRGB.keys())
keys_list = sorted(keys_list, key=lambda x: int(x.split('_')[1]))
color_num = len(keys_list)
...
for key_index in range(len(keys_list)):
    ...
    specbos_xyY_linear = np.array([specbos_measure['x_mean_filter'], specbos_measure['y_mean_filter'], specbos_measure['Y_mean_filter']])
...
with open(f'RGB_camera_XYZ_specbos_linear.json', 'w') as f:
...
with open(f'Camera_Colorchecker_RGB2XYZ.json', 'w') as f:
